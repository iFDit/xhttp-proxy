...
const core = require('./thunk')
const httpStream = require('./http-stream').default
const source = require('./stream-source/xmlhttprequest')

function enable() {
  const requeststream = new httpStream.HttpReadStream({
    type: 'XHRReadStream',
    observable$: source.xhrReqObservable$,
  })
  const responsestream = new httpStream.HttpWriteStream({
    type: 'XHRWriteStream',
    observable$: source.xhrResObservable$,
  })
  core.thunk(requeststream, responsestream)
  source.enable()
